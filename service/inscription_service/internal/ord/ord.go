...
import (
	...
	extRpcClient "self-tool/service/inscription_service/pkg/rpcclient"
	"strings"
	"time"
...
OP_IF
	...
	<metaid_flag> // Push "metaid" bytecode
	<operation>  // "init/create/modify/revoke"
	<path> // 要操作的路径 /protocols/simplebuzz/1
	<encryption> //content的加密类型，0为不加密；1为ECIES加密，2为ECDH协商密钥加密。默认为0不加密。
	<version>  //版本号
	<content-type> // 必选，payload的文件格式，默认为application/jason，包含了payload的编码方式
	<payload>    // Payload
...
type InscriptionRequest struct {
	...
	MetaIdDataList     []InscriptionMetaIdData
...
func NewInscriptionTool(net *chaincfg.Params, rpcclient *rpcclient.Client, request *InscriptionRequest) (*InscriptionTool, error) {
	tool := &InscriptionTool{
		...
		txCtxDataList:             make([]*inscriptionTxCtxData, len(request.MetaIdDataList)),
...
func (tool *InscriptionTool) _initTool(net *chaincfg.Params, request *InscriptionRequest) error {
	destinations := make([]string, len(request.MetaIdDataList))
	revealOutValue := defaultRevealOutValue
	if request.RevealOutValue > 0 {
		revealOutValue = request.RevealOutValue
	}
	for i := 0; i < len(request.MetaIdDataList); i++ {
		var (
			txCtxData *inscriptionTxCtxData
			err       error
		)
		if request.MetaIdDataList != nil && len(request.MetaIdDataList) != 0 {
			txCtxData, err = createMetaIdTxCtxData(net, request, i)
		} else {
		...
		destinations[i] = request.MetaIdDataList[i].Destination
...
OP_IF
	...
	<metaid_flag> // Push "metaid" bytecode
	<operation>  // "init/create/modify/revoke"
	<path> // 要操作的路径 /root/protocols/SimpleBuzz
	<content>    // Payload
	<encryption> //content的加密类型，0为不加密；1为ECIES加密，2为ECDH协商密钥加密。默认为0不加密。
	<version>  //版本号
	<content-type> // 可选，content的文件格式，默认为application/jason
	<encoing> //可选，编码方式，默认为UTF-8

OP_ENDIF
*/
func createMetaIdTxCtxData(net *chaincfg.Params, inscriptionRequest *InscriptionRequest, indexOfRequestDataList int) (*inscriptionTxCtxData, error) {
	privateKey, err := btcec.NewPrivateKey()
	if err != nil {
		return nil, err
	}
	metaIdData := inscriptionRequest.MetaIdDataList[indexOfRequestDataList]
	inscriptionBuilder := txscript.NewScriptBuilder().
		AddData(schnorr.SerializePubKey(privateKey.PubKey())).
		AddOp(txscript.OP_CHECKSIG).
		AddOp(txscript.OP_FALSE).
	...
	if strings.ToLower(metaIdData.Operation) != "init" {
